
<%@ page import="dcc.AppConfig" %>
<!DOCTYPE html>
<html>
	<head>
		<meta name="layout" content="main">
		<g:set var="entityName" value="${message(code: 'appConfig.label', default: 'AppConfig')}" />
		<title><g:message code="default.list.label" args="[entityName]" /></title>
	</head>
	<body>
		<a href="#list-appConfig" class="skip" tabindex="-1"><g:message code="default.link.skip.label" default="Skip to content&hellip;"/></a>
		<div class="nav" role="navigation">
			<ul>
				<li><a class="home" href="${createLink(uri: '/')}"><g:message code="default.home.label"/></a></li>
				<li><g:link class="create" action="create"><g:message code="default.new.label" args="[entityName]" /></g:link></li>
			</ul>
		</div>
		<div id="list-appConfig" class="content scaffold-list" role="main">
			<h1><g:message code="default.list.label" args="[entityName]" /></h1>
			<g:if test="${flash.message}">
			<div class="message" role="status">${flash.message}</div>
			</g:if>
			<table>
				<thead>
					<tr>
					
						<g:sortableColumn property="configKey" title="${message(code: 'appConfig.configKey.label', default: 'Config Key')}" />
					
						<g:sortableColumn property="remark" title="${message(code: 'appConfig.remark.label', default: 'Remark')}" />
					
						<g:sortableColumn property="configValue" title="${message(code: 'appConfig.configValue.label', default: 'Config Value')}" />
					
						<g:sortableColumn property="dateCreated" title="${message(code: 'appConfig.dateCreated.label', default: 'Date Created')}" />
					
						<g:sortableColumn property="lastUpdated" title="${message(code: 'appConfig.lastUpdated.label', default: 'Last Updated')}" />
					
					</tr>
				</thead>
				<tbody>
				<g:each in="${appConfigInstanceList}" status="i" var="appConfigInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
					
						<td><g:link action="show" id="${appConfigInstance.id}">${fieldValue(bean: appConfigInstance, field: "configKey")}</g:link></td>
					
						<td>${fieldValue(bean: appConfigInstance, field: "remark")}</td>
					
						<td>${fieldValue(bean: appConfigInstance, field: "configValue")}</td>
					
						<td><g:formatDate date="${appConfigInstance.dateCreated}" /></td>
					
						<td><g:formatDate date="${appConfigInstance.lastUpdated}" /></td>
					
					</tr>
				</g:each>
				</tbody>
			</table>
			<div class="pagination">
				<g:paginate total="${appConfigInstanceTotal}" />
			</div>
		</div>
	</body>
</html>
